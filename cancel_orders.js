...
const PRIVATE_KEY = '5ytlo2WFLHwSP3FoChdSASe8u2SYyKQBa4o9GSEbPrtddRK2GPt1gFHyq'
...
const fetchOpenOrders = async (username) => {
    const openOrders = await fetchFromAPI(apiRoot, `/v1/orders/open?limit=250&offset=0&account=${username}`);
    return openOrders;
  };
...
const main = async () => {
  ...
  if(!marketSymbol) {
      ...
      const market = allMarkets.filter((markets) => markets.symbol === marketSymbol);
      if (market === undefined) {
        throw new Error(`Market ${marketSymbol} does not exist`);
      }
      const marketOrders = orders.filter((orders) => orders.market_id === market[0].market_id);
